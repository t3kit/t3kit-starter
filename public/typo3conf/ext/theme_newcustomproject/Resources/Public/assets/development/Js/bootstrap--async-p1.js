(function () {
  ...
  const getElementFromSelector$1 = element => {
  ...
  const typeCheckConfig$1 = (componentName, config, configTypes) => {
    Object.keys(configTypes).forEach(property => {
      const expectedTypes = configTypes[property];
      const value = config[property];
  ...
  const Data$1 = {
    setData(instance, key, data) {
      mapData$1.set(instance, key, data);
    },
    getData(instance, key) {
      return mapData$1.get(instance, key)
    },
    removeData(instance, key) {
      mapData$1.delete(instance, key);
    }
  };
  ...
  function getUidEvent$1(element, uid) {
    return (uid && `${uid}::${uidEvent$1++}`) || element.uidEvent || uidEvent$1++
  }

  function getEvent$1(element) {
    const uid = getUidEvent$1(element);
  ...
  function normalizeParams$1(originalTypeEvent, handler, delegationFn) {
    ...
    const isNative = nativeEvents$1.has(typeEvent);
  ...
  function addHandler$1(element, originalTypeEvent, handler, delegationFn, oneOff) {
    if (typeof originalTypeEvent !== 'string' || !element) {
      return
    }
    ...
    const events = getEvent$1(element);
    const handlers = events[typeEvent] || (events[typeEvent] = {});
    ...
    const uid = getUidEvent$1(originalHandler, originalTypeEvent.replace(namespaceRegex$1, ''));
    const fn = delegation ?
      bootstrapDelegationHandler$1(element, handler, delegationFn) :
      bootstrapHandler$1(element, handler);
  ...
  function removeNamespacedHandlers$1(element, events, typeEvent, namespace) {
    ...
    Object.keys(storeElementEvent).forEach(handlerKey => {
      if (handlerKey.includes(namespace)) {
        ...
        removeHandler$1(element, events, typeEvent, event.originalHandler, event.delegationSelector);
      }
    });
  }
  ...
  const EventHandler$1 = {
    on(element, event, handler, delegationFn) {
      addHandler$1(element, event, handler, delegationFn, false);
    },

    one(element, event, handler, delegationFn) {
      addHandler$1(element, event, handler, delegationFn, true);
    },
    ...
    off(element, originalTypeEvent, handler, delegationFn) {
      ...
      if (typeof originalHandler !== 'undefined') {
        ...
        removeHandler$1(element, events, typeEvent, originalHandler, delegation ? handler : null);
        return
      }
  ...
  const SelectorEngine$1 = {
    ...
    parents(element, selector) {
      ...
      while (ancestor && ancestor.nodeType === Node.ELEMENT_NODE && ancestor.nodeType !== NODE_TEXT$1) {
        if (this.matches(ancestor, selector)) {
          parents.push(ancestor);
        }
  ...
  class BaseComponent$1 {
    constructor(element) {
      ...
      this._element = element;
      Data$1.setData(element, this.constructor.DATA_KEY, this);
    }
    ...
    static getInstance(element) {
      return Data$1.getData(element, this.DATA_KEY)
    }
  ...
  class Dropdown extends BaseComponent$1 {
    ...
    show() {
      ...
      const showEvent = EventHandler$1.trigger(this._element, EVENT_SHOW$1, relatedTarget);
      ...
      if ('ontouchstart' in document.documentElement &&
        ...
        [].concat(...document.body.children)
          .forEach(elem => EventHandler$1.on(elem, 'mouseover', null, noop()));
      }
      ...
      EventHandler$1.trigger(parent, EVENT_SHOWN$1, relatedTarget);
    }
    ...
    hide() {
      ...
      const hideEvent = EventHandler$1.trigger(parent, EVENT_HIDE$1, relatedTarget);
      ...
      this._menu.classList.toggle(CLASS_NAME_SHOW$1);
      this._element.classList.toggle(CLASS_NAME_SHOW$1);
      EventHandler$1.trigger(parent, EVENT_HIDDEN$1, relatedTarget);
    }

    dispose() {
      super.dispose();
      EventHandler$1.off(this._element, EVENT_KEY$1);
      this._menu = null;
    ...
    _addEventListeners() {
      EventHandler$1.on(this._element, EVENT_CLICK, event => {
        event.preventDefault();
    ...
    _getConfig(config) {
      ...
      typeCheckConfig$1(NAME$1, config, this.constructor.DefaultType);
    ...
    _getMenuElement() {
      return SelectorEngine$1.next(this._element, SELECTOR_MENU)[0]
    }
    ...
    static dropdownInterface(element, config) {
      let data = Data$1.getData(element, DATA_KEY$1);
    ...
    static clearMenus(event) {
      ...
      const toggles = SelectorEngine$1.find(SELECTOR_DATA_TOGGLE$1);

      for (let i = 0, len = toggles.length; i < len; i++) {
        const parent = Dropdown.getParentFromElement(toggles[i]);
        const context = Data$1.getData(toggles[i], DATA_KEY$1);
        const relatedTarget = {
        ...
        if ('ontouchstart' in document.documentElement) {
          [].concat(...document.body.children)
            .forEach(elem => EventHandler$1.off(elem, 'mouseover', null, noop()));
        }
    ...
    static getParentFromElement(element) {
      return getElementFromSelector$1(element) || element.parentNode
    }
    ...
    static dataApiKeydownHandler(event) {
      ...
      if (event.key === ESCAPE_KEY) {
        const button = this.matches(SELECTOR_DATA_TOGGLE$1) ? this : SelectorEngine$1.prev(this, SELECTOR_DATA_TOGGLE$1)[0];
        button.focus();
        Dropdown.clearMenus();
        return
      }
      ...
      const items = SelectorEngine$1.find(SELECTOR_VISIBLE_ITEMS, parent).filter(isVisible);
  ...
  EventHandler$1.on(document, EVENT_KEYDOWN_DATA_API, SELECTOR_DATA_TOGGLE$1, Dropdown.dataApiKeydownHandler);
  EventHandler$1.on(document, EVENT_KEYDOWN_DATA_API, SELECTOR_MENU, Dropdown.dataApiKeydownHandler);
  EventHandler$1.on(document, EVENT_CLICK_DATA_API$1, Dropdown.clearMenus);
  ...
  EventHandler$1.on(document, EVENT_CLICK_DATA_API$1, SELECTOR_FORM_CHILD, e => e.stopPropagation());
